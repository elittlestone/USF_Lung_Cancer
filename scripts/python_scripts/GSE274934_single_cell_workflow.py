...
def main():
    ...
    output_directory = "results/GSE274934"
    ...
    if args.skip_scvi_training:
        ...
        degs_df = differential_gene_expression(adata_for_celltypist, vae_file, csv_output_dir)
        generate_degs_figures(adata_for_celltypist, degs_df)
    else:
...
def differential_gene_expression(adata_for_celltypist, vae, csv_output_dir, delta = 0.5):
    """
    Perform scVI Differential Expression for each celltype vs rest 
    """


    print("Loading trained scVI model...")
    vae = scvi.model.SCVI.load(vae, adata = adata_for_celltypist)
     
    os.makedirs(csv_output_dir, exist_ok = True)
    
    celltype_key = "celltypist_predicted_labels"
    celltypes = adata_for_celltypist.obs[celltype_key].unique().tolist()
    ...
    for celltype in celltypes:
        ...
        differential_expr_results = vae.differential_expression(
                ...
                group1 = celltype,
                group2 = None, # vs all other celltypes 
                mode = "change",
                delta = delta,
                )
    ...
    all_differential_expr_results_dataframe.to_csv(os.path.join(csv_output_dir, "scvi_all_vs_rest_DGEA.csv"))
    print("scVI DGEA completed!") 
    return all_differential_expr_results_dataframe

    
def generate_degs_figures(adata_for_celltypist, degs_df, top_n = 5):
    """
    Generate figures for visualizing top DEGs per celltype 
    """
    ...
    top_genes = (
            degs_df
            .sort_values(by=["celltype", sort_col], ascending = [True, False])
            .groupby("celltype")
            .head(top_n)
            .index
            .unique()
